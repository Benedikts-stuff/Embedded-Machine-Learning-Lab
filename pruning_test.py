...
def run_pruning_study(save_path_plot, device, criterion, loader_new, train_loader):
    ...
    ap_base = evaluate_person_accuracy(base_model, 1, device, loader_new, num_samples=500)
    inf_time_base = np.mean(run_comparison_benchmark_person({"base": base_model}, device, num_samples=250)["base"])
    ...
    for target_pruned_pct in targets:
        ...
        stats = run_comparison_benchmark_person({"pruned": pruned_model}, device, num_samples=250)
...
def run_quantization_pipeline(float_model, device, loader_new):
    ...
    stats_q = run_comparison_benchmark_person({"Quantized (INT8)": q_model}, 'cpu', num_samples=1000)
    inf_time_q = np.mean(stats_q["Quantized (INT8)"])
    ap_q = evaluate_person_accuracy(q_model, 1, 'cpu', loader_new, num_samples=1000)
...
if __name__ == "__main__":
    ...
    percent_steps = [10, 20, 30, 40, 50, 60, 70]
    sweep_data = []
    ...
    for pct in percent_steps:
        ...
        ap_fp32 = evaluate_person_accuracy(model_fp32, 1, device_quantized, loader_new, num_samples=1000)
        stats_f = run_comparison_benchmark_person({"FP32": model_fp32}, device_quantized, num_samples=1000)
